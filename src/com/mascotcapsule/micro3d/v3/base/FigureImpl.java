/*
 * Copyright 2022 Yury Kharchenko (original KEmulator implementation)
 *
 ...
 *
 * Note: the original FigureNode pool (RenderNode.FigureNode stack) is backend-specific.
 * Here the engine data layer exposes fillBuffers() / setPosture() / setPattern() which
 * any Micro3dBackend consumes; per-frame node pooling is handled inside the backend.
 */
...
public class FigureImpl {
	...
	private void fillTexCoordBuffer() {
		java.nio.ByteBuffer buffer = model.texCoordArray;
		buffer.rewind();
