/*
 * Copyright 2020-2024 Yury Kharchenko (original KEmulator implementation)
 *
 ...
 * Pure JDK: point-sprite vertex expansion, matrix-vector math, CPU bone skinning.
 * The emulator.Emulator.version dependency was dropped.
 */
...
public class Utils {
	...
	static void transform(FloatBuffer srcVertices, FloatBuffer dstVertices,
		...
		// The bones buffer is a direct ByteBuffer written by Loader in its own
		// (native/little-endian) order, mixing ints and floats. We MUST read it
		// through the ByteBuffer itself (with its order), NOT through asIntBuffer()/
		// asFloatBuffer() views — those views default to BIG_ENDIAN regardless of
		// the backing buffer's order, which would mis-decode the data on LE hosts
		// (observed: all vertices collapse to one point).
		float[] boneMatrixTmp = new float[bonesLen * 12];
		float[] tmp = new float[12];
		int boneStride = (12 + 2) * 4; // bytes per bone record
		for (int i = 0; i < bonesLen; ++i) {
			int base = i * boneStride;
			int boneLen = bonesBuf.getInt(base);
			int parent = bonesBuf.getInt(base + 4);
			int mtxBase = base + 8; // 2 ints
			if (parent == -1) {
				for (int k = 0; k < 12; k++) {
